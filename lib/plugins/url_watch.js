  ...
  Plugin = (function() {
    ...
    Plugin.prototype.urlDetails = function(channel, who, message, url) {
      ...
      request = new UrlFetcher(url).handle(function(res) {
        var content_type, data, length, location, status_code;
        ...
        res.on('end', function() {
          ...
          if (location) {
            return _this.bot.say(channel, "Url | " + display_status + " (" + content_type + ") | " + location);
          } else if (title && title[1]) {
            return _this.bot.say(channel, "Url | " + display_status + " (" + content_type + ") | " + (("" + title[1]).replace(/&amp;/, "&")));
          } else if (length) {
