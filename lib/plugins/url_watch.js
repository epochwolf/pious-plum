  ...
  Plugin = (function() {
    ...
    Plugin.prototype.urlDetails = function(channel, who, message, url) {
      ...
      request = new UrlFetcher(url).handle(function(res) {
        var content_type, data, length, new_url, status_code;
        ...
        data = "";
        new_url = ("" + url.host + url.path + (url.query || "")) !== ("" + res.req._headers.host + res.req.path) ? "" + res.req._headers.host + res.req.path : void 0;
        if (new_url) {
          _this.bot.emit("message_with_url:" + res.req._headers.host, channel, who, message, UrlDetector.has_url("http://" + new_url));
        }
        res.on('end', function() {
